...
def time_series_kfold_split(data, n_splits):
    """
    Time series aware k-fold split, each fold's training set is continuous
    
    For example, when k=7:
    - fold 0: validation=[0], training=[1,2,3,4,5,6]
    - fold 1: validation=[1], training=[2,3,4,5,6]
    - fold 2: validation=[2], training=[3,4,5,6]
    - fold 3: validation=[3], training=[0,1,2]
    - fold 4: validation=[4], training=[0,1,2,3]
    - fold 5: validation=[5], training=[0,1,2,3,4]
    - fold 6: validation=[6], training=[0,1,2,3,4,5]
    ...
    for i in range(n_splits):
        # validation set is continuous
        val_start = i * fold_size
        val_end = val_start + fold_size if i < n_splits - 1 else n_samples
        val_indices = list(range(val_start, val_end))
        
        # Calculate the length of the continuous segment on the left and right
        left_size = val_start  # The length of the continuous segment on the left
        right_size = n_samples - val_end  # The length of the continuous segment on the right
        
        # Select the longer continuous segment as the training set
        if left_size >= right_size:
            # Use the left continuous segment
            train_start = 0
            train_end = val_start
            train_indices = list(range(train_start, train_end))
        else:
            # Use the right continuous segment
            train_start = val_end
            train_end = n_samples
            train_indices = list(range(train_start, train_end))
...
def run_vcdf(data, base_method, n_splits=5, consistency_threshold=0.4, 
    ...
    # Use time series aware k-fold splitting instead of standard KFold
